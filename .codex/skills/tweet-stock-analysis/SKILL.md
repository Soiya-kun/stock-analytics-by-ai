...
When the user asks for a report, prefer a curated 5-10 name output that explains who mentioned each stock and how they framed it, instead of dumping every persisted row.
...
4. Before analyzing recent dates, run `docker compose run --rm xcollector ensure-current`. Add `--target-username USERNAME` only when the user explicitly asks to limit analysis to one monitored account.
5. Treat the tweet set as current when each target's `ingest.x_timeline_state.last_success_at` is within the last 60 minutes. If it is older or missing, let `ensure-current` fetch only the incremental gap.
6. Prepare a range-scoped analysis template with `docker compose run --rm analysis prepare-tweet-analysis ...`. Omit `--target-username` by default so the template includes all DB-monitored targets.
...
15. Build the final report around roughly 5-10 stocks unless the user asks for full output. For each highlighted stock, include the usernames, the tone or thesis of the mentions, and whether price/volume context supported the excitement.
16. Persist with `docker compose run --rm analysis persist-tweet-analysis --input-file ...`.
...
- For current-day or near-real-time analysis, always perform the freshness check first and skip X API calls when the last successful poll is already within the 60-minute budget window. Run it across all active DB-monitored accounts unless the user explicitly requested a single target.
...
- In the final report, do not stop at symbol names and counts. Explain the actual stance behind the tweet, such as breakout expectation,âüÇµñ⁄îÉÇ¢, åàéZä˙ë“, çﬁóøévòf, valuation comment, or simple watchlist mention.
- Prefer a selective report over exhaustive output. If there are many names, compress into the 5-10 most decision-useful symbols by combining cross-user confirmation, mention intensity, and the clarity/strength of the thesis.
