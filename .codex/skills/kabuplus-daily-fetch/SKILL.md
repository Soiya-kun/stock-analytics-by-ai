...
Use this skill to fetch `japan-all-stock-prices/daily` CSV files through the repository's guarded workflow. The workflow always checks the directory listing first, saves confirmed files under `stock/kabuplus-2026`, and then imports them through the existing Docker importer. Run both the downloader script and the importer through `docker compose`; do not rely on a locally installed Python runtime for daily imports.
...
4. Run `docker compose run --rm -e CSVEX_BASIC_USER -e CSVEX_BASIC_PASSWORD --entrypoint python importer /workspace/scripts/fetch_kabuplus_daily_csv.py --date ... --output-dir /workspace/stock/kabuplus-2026`.
...
- Use `docker compose` for the downloader as well as the importer so the workflow does not depend on local Python packages.
